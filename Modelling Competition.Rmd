...
Lets correct for the categories that dont appear in train validation for Exterior1st
...
outliers_indices <- which(abs(model_interactions$residuals) > 3 * sd(model_interactions$residuals))
...
```{r}
RMSE(model_outliers$fitted.values, train_train$SalePrice)
```
...
```{r}
model_new=update(model_outliers, .~.- Functional- Exterior1st)
summary(model_new)

RMSE(model_new$fitted.values, train_train[-outliers_indices,]$SalePrice)
