...
train$GarageYrBlt <- ifelse(is.na(train$GarageYrBlt),round(mean(train$GarageYrBlt,na.rm=TRUE)),train$GarageYrBlt)
train$MasVnrArea <- ifelse(is.na(train$MasVnrArea),round(mean(train$MasVnrArea,na.rm=TRUE)),train$MasVnrArea)
train$Electrical <- ifelse(is.na(train$Electrical),"SBrkr",train$Electrical) #Para esta y abajo he usado
train$MasVnrType <- ifelse(is.na(train$MasVnrType),"None",train$MasVnrType) #los valores extraidos abajo
```
...
```{r}
#We extract a dataset only with
...
It would be useful to analyse the categorical varaibles, seeing most popular categories for each variables.
