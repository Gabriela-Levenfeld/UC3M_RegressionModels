...
interaction_matrix <- matrix(0, ncol = ncol(train), nrow = ncol(train), dimnames = list(names(train), names(train)))

# Loop through all pairs of variables
for (i in 1:(ncol(train) - 1)) {
  variable1 <- names(train)[i]
  for (j in (i + 1):ncol(train)) {
    variable2 <- names(train)[j]
    ...
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars +", variable1, ":", variable2)), data = train)
      lmod_without_inter <- lm(as.formula(paste(dependent_variable, "~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
...
significant_interactions <- which(anova_05 < 0.005 & anova_05>0, arr.ind = TRUE)
...
modelovif2_outliers<- lm(formula = SalePrice ~ BsmtFinSF1 + TotalBsmtSF + 
    ...
    dummy_garage:GarageCars, data = train[-c(524,629,363),])
plot(modelovif3_outliers)
