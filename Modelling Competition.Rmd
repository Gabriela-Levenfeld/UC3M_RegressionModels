...
```{r}
model2 <- lm(SalePrice~LotFrontage + LotArea + YearBuilt + dummy_RemodYear:YearRemodAdd + dummy_masvnr:MasVnrArea+dummy_masvnr:MasVnrType+ BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + X1stFlrSF + X2ndFlrSF + GrLivArea +dummy_garage:GarageCars+dummy_garage:GarageYrBlt+dummy_garage:GarageArea+dummy_garage:GarageType+dummy_garage:GarageFinish+dummy_garage:GarageCond+ WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + YrSold + BsmtFullBath + BsmtHalfBath + FullBath+ HalfBath + BedroomAbvGr+ TotRmsAbvGrd+ Fireplaces + MoSold + MSZoning + LotShape + LandContour + LotConfig + Neighborhood + BldgType  + OverallQual + OverallCond + Exterior1st + ExterQual + ExterCond  + BsmtFinType2 + HeatingQC  + KitchenQual+ BsmtCond+LandSlope+Condition1+RoofMatl,data=train)
alias(model2)
```
...
vif(modelo2aic)
```
We delete GrLivArea . GVIF^(1/(2*Df))=4.792750 . We have to compare with $\sqrt{10}\approx$ 3.2

```{r}
modelovif1<- lm(formula = SalePrice ~ LotArea + TotalBsmtSF + X2ndFlrSF +
                  OpenPorchSF + EnclosedPorch + ScreenPorch + BsmtFullBath + 
    BsmtHalfBath + HalfBath + BedroomAbvGr + LandContour + Neighborhood + 
    BldgType + OverallQual + OverallCond + Exterior1st + ExterCond + 
    HeatingQC + Condition1 + RoofMatl + dummy_RemodYear:YearRemodAdd + 
    dummy_masvnr:MasVnrArea + dummy_masvnr:MasVnrType + dummy_garage:GarageArea + 
    dummy_garage:GarageCond, data = train)
vif(modelovif1)
```
We decide to keep LotArea even though the GVIF^(1/(2*Df))=3.353705. We have to compare with $\sqrt{10}\approx$ 3.2
...
name_vars <- c('LotArea' , 'TotalBsmtSF' , 'X2ndFlrSF' , 
    'OpenPorchSF' , 'EnclosedPorch' , 'ScreenPorch' , 'BsmtFullBath' , 
    'BsmtHalfBath' , 'HalfBath' , 'BedroomAbvGr' , 'LandContour' , 'Neighborhood' , 
    'BldgType' , 'OverallQual' , 'OverallCond' , 'Exterior1st' , 'ExterCond' , 
    'HeatingQC' , 'Condition1' , 'RoofMatl' , 'dummy_RemodYear:YearRemodAdd' , 
    'dummy_masvnr:MasVnrArea' , 'dummy_masvnr:MasVnrType' , 'dummy_garage:GarageArea' , 
    'dummy_garage:GarageCond')
...
for (i in 1:(length(name_vars))) {
  ...
  for (j in (i): length(name_vars)) {
    ...
    if (variable1 != dependent_variable && variable2 != dependent_variable && variable1 != variable2) {
      # Run both models
      lmod_all_vars <- lm(as.formula(paste(dependent_variable, "~ LotArea + TotalBsmtSF + X2ndFlrSF + 
    OpenPorchSF + EnclosedPorch + ScreenPorch + BsmtFullBath + 
    BsmtHalfBath + HalfBath + BedroomAbvGr + LandContour + Neighborhood + 
    BldgType + OverallQual + OverallCond + Exterior1st + ExterCond + 
    HeatingQC + Condition1 + RoofMatl + dummy_RemodYear:YearRemodAdd + 
    dummy_masvnr:MasVnrArea + dummy_masvnr:MasVnrType + dummy_garage:GarageArea + 
    dummy_garage:GarageCond +", variable1, ":", variable2)), data = train)
      lmod_without_inter <- lm(as.formula(paste(dependent_variable, "~ LotArea + TotalBsmtSF + X2ndFlrSF + 
    OpenPorchSF + EnclosedPorch + ScreenPorch + BsmtFullBath + 
    BsmtHalfBath + HalfBath + BedroomAbvGr + LandContour + Neighborhood + 
    BldgType + OverallQual + OverallCond + Exterior1st + ExterCond + 
    HeatingQC + Condition1 + RoofMatl + dummy_RemodYear:YearRemodAdd + 
...
significant_interactions <- which(anova_05 < 0.05 & anova_05>0, arr.ind = TRUE)
...
+TotalBsmtSF : Neighborhood +HalfBath : Neighborhood +EnclosedPorch : OverallCond +

```{r}
modelovif1_interactions <- lm(formula= SalePrice~ LotArea + TotalBsmtSF + X2ndFlrSF + 
    OpenPorchSF + EnclosedPorch + ScreenPorch + BsmtFullBath + 
    BsmtHalfBath + HalfBath + BedroomAbvGr + LandContour + Neighborhood + 
    BldgType + OverallQual + OverallCond + Exterior1st + ExterCond + 
    HeatingQC + Condition1 + RoofMatl + dummy_RemodYear:YearRemodAdd + 
    dummy_masvnr:MasVnrArea + dummy_masvnr:MasVnrType + dummy_garage:GarageArea + 
    dummy_garage:GarageCond +TotalBsmtSF : Neighborhood +HalfBath : Neighborhood +EnclosedPorch : OverallCond, data = train)

summary(modelovif1_interactions)
modelovif1_interactions_stepAIC <- stepAIC(modelovif1_interactions, direction = 'both', trace=FALSE)
summary(modelovif1_interactions_stepAIC)
```
```{r}
anova(modelovif1, modelovif1_interactions)
```
...
```{r, cache=FALSE, message=FALSE,warning=FALSE}
resact <- data.frame(residual = modelovif1$residuals, fitted = modelovif1$fitted.values)
