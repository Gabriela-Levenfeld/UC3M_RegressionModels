...
categorias_diferentes <- obtener_diferencia_categorias(train[,33:70], test[,32:69])
...
```{r}
model1 <- lm(SalePrice~LotFrontage + LotArea + YearBuilt + dummy_RemodYear:YearRemodAdd + dummy_masvnr:MasVnrArea+dummy_masvnr:MasVnrType+ BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + X1stFlrSF + X2ndFlrSF + GrLivArea +dummy_garage:GarageCars+dummy_garage:GarageYrBlt+dummy_garage:GarageArea+dummy_garage:GarageType+dummy_garage:GarageFinish+dummy_garage:GarageCond+ WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + YrSold + BsmtFullBath + BsmtHalfBath + FullBath+ HalfBath + BedroomAbvGr+ KitchenAbvGr + TotRmsAbvGrd+ Fireplaces + MoSold + MSSubClass+ MSZoning + LotShape + LandContour + LotConfig + Neighborhood + BldgType + HouseStyle + OverallQual + OverallCond + Exterior2nd + RoofStyle + Exterior1st + ExterQual + ExterCond + Foundation + BsmtQual + BsmtExposure + BsmtFinType1 + BsmtFinType2 + HeatingQC + Electrical + KitchenQual+ Functional+ FireplaceQu+RoofMatl+Condition1+CentralAir+Heating+BsmtCond+LandSlope+CentralAir+Condition1+RoofMatl,data=train)
```

*We delete the variables that are correlated between them:*
*Next, we have noticed during the assignment that some variables have a correlation BsmtFinType1 and YearRemode (deleting BsmtFinType1- Pero el modelo con ambas funciona), Exterior1st and Exterior2nd (deleting Exterior2nd), MSSubClass and BldgType (deleting MSSUbClass),BsmtCond and BsmtQual(deleting BsmtCond), Functional and dummy_RemodYear:YearRemodAdd also cause errors due to multicollinearity if added to the model.*

```{r}
model2 <- lm(SalePrice~LotFrontage + LotArea + YearBuilt+ dummy_masvnr:MasVnrArea+dummy_masvnr:MasVnrType+BsmtFinSF1+BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + X1stFlrSF + X2ndFlrSF + GrLivArea +dummy_garage:GarageCars+dummy_garage:GarageYrBlt+dummy_garage:GarageArea+dummy_garage:GarageType+dummy_garage:GarageFinish+dummy_garage:GarageCond+ WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + YrSold + BsmtFullBath + BsmtHalfBath + FullBath+ HalfBath + BedroomAbvGr+ KitchenAbvGr + TotRmsAbvGrd+ Fireplaces + MoSold + MSZoning + LotShape + LandContour + LotConfig + Neighborhood + BldgType + HouseStyle + OverallQual + OverallCond + RoofStyle + Exterior1st + ExterQual + ExterCond + Foundation + BsmtQual + BsmtExposure + BsmtFinType2 + HeatingQC + Electrical + KitchenQual+ FireplaceQu+LandSlope+CentralAir+Condition1+RoofMatl, data=train)
```

The following step is to use step AIC.

```{r}
modelo2aic <- stepAIC(model2,direction="both", trace=FALSE)
summary(modelo2aic)
```
...
vif(modelo2aic)
```
We delete YearBuilt. GVIF^(1/(2*Df))=3.909867 . We have to compare with $\sqrt{10}\approx$ 3.2

```{r}
modelovif1<- lm(formula = SalePrice ~ LotArea + BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars, 
    data = train)
vif(modelovif1)
```
We delete LotArea. GVIF^(1/(2*Df))=3.661776 . We have to compare with $\sqrt{10}\approx$ 3.2

```{r}
modelovif2<- lm(formula = SalePrice ~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
...
name_vars <- c('BsmtFinSF1','TotalBsmtSF','GrLivArea','OpenPorchSF',
    'EnclosedPorch','X3SsnPorch','ScreenPorch','BsmtFullBath',
    'TotRmsAbvGrd','Fireplaces','LotShape','LandContour','LotConfig',
    'Neighborhood','BldgType','OverallQual','OverallCond','ExterCond',
    'BsmtQual','BsmtExposure','Electrical','KitchenQual','Condition1',
    'RoofMatl','dummy_masvnr:MasVnrArea','dummy_garage:GarageCars')
...
for (i in 1:(length(name_vars))) {
  ...
  for (j in (i): length(name_vars)) {
    ...
    if (variable1 != dependent_variable && variable2 != dependent_variable && variable1 != variable2) {
      # Run both models
      lmod_all_vars <- lm(as.formula(paste(dependent_variable, "~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars +", variable1, ":", variable2)), data = train)
      lmod_without_inter <- lm(as.formula(paste(dependent_variable, "~ BsmtFinSF1 +TotalBsmtSF + GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars -", variable1, ":", variable2)), data = train)
...
```{r}
modelovif2_interactions <- lm(formula= SalePrice~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars + TotalBsmtSF : GrLivArea +TotalBsmtSF : LandContour +GrLivArea : LandContour +OpenPorchSF : LandContour +TotRmsAbvGrd : LandContour +TotalBsmtSF : Neighborhood +GrLivArea : Neighborhood +TotRmsAbvGrd : Neighborhood +GrLivArea : OverallQual +TotRmsAbvGrd : ExterCond  +GrLivArea : KitchenQual +TotalBsmtSF : Condition1 +GrLivArea : Condition1 +LandContour : dummy_masvnr:MasVnrArea +Neighborhood : dummy_masvnr:MasVnrArea +LandContour : dummy_garage:GarageCars +Condition1 : dummy_garage:GarageCars , data=train)

summary(modelovif2_interactions)
modelovif2_interactions_stepAIC <- stepAIC(modelovif2_interactions, direction = 'both', trace=FALSE)
summary(modelovif2_interactions_stepAIC)
```
```{r}
modelovif2_interactions2 <- lm(formula= SalePrice~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars + Condition1:dummy_garage:GarageCars, data=train)

anova(modelovif2, modelovif2_interactions2)
```
TotalBsmtSF:GrLivArea (si) + TotalBsmtSF:LandContour (si) + GrLivArea:LandContour (si) + OpenPorchSF:LandContour (si) + GrLivArea:OverallQual (si) + TotRmsAbvGrd:ExterCond (si)  + TotalBsmtSF:Condition1 (si) +GrLivArea:Condition1 (si)+ Condition1:dummy_garage:GarageCars (si)


```{r}
modelovif2_interactions3 <- lm(formula= SalePrice~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCars + TotalBsmtSF:GrLivArea + TotalBsmtSF:LandContour + GrLivArea:LandContour + OpenPorchSF:LandContour + GrLivArea:OverallQual + TotRmsAbvGrd:ExterCond  + TotalBsmtSF:Condition1 +GrLivArea:Condition1 + Condition1:dummy_garage:GarageCars , data=train)

summary(modelovif2_interactions3)

anova(modelovif2, modelovif2_interactions3)

modelovif2_interactions_stepAIC2 <- stepAIC(modelovif2_interactions3, direction = 'both', trace=FALSE)
summary(modelovif2_interactions_stepAIC2)
...
```{r}
a<-predict(modelovif2_interactions2, data=test)
RMSE(a, train$SalePrice)
...
```{r, cache=FALSE, message=FALSE,warning=FALSE}
resact <- data.frame(residual = modelovif2$residuals, fitted = modelovif2$fitted.values)
...
```{r}
shapiro.test(modelovif2$residuals)
qqnorm(residuals(modelovif2))
qqline(residuals(modelovif2))
```
...
```{r}
anova(modelovif2,model2)
```

Interpretation: Model without so many variables fits better. The RSS decrease much, from 0.68 to 0.58.


```{r}
confint(modelovif2,level=0.95)
```
...
The outliers are: 524, 633, 1325. We are going to remove them.
The influencial observations are: 524, 629, 1152 (we only delete 524 because is out of Cook's distance)

```{r}
modelovif2_outliers<- lm(formula = SalePrice ~ BsmtFinSF1 + TotalBsmtSF + 
    GrLivArea + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + 
    BsmtFullBath + TotRmsAbvGrd + Fireplaces + LotShape + LandContour + 
    LotConfig + Neighborhood + BldgType + OverallQual + OverallCond + 
    ExterCond + BsmtQual + BsmtExposure + Electrical + KitchenQual + 
    Condition1 + RoofMatl + dummy_masvnr:MasVnrArea + dummy_garage:GarageCarsBsmtFinSF1 + TotalBsmtSF + 
    X1stFlrSF + X2ndFlrSF + OpenPorchSF + EnclosedPorch + 
    X3SsnPorch + ScreenPorch + BsmtFullBath + TotRmsAbvGrd + 
    Fireplaces + MSZoning + LotShape + LandContour + LotConfig + 
    Neighborhood + BldgType + OverallQual + OverallCond + RoofStyle + 
    BsmtQual + BsmtExposure + Electrical + KitchenQual + dummy_masvnr:MasVnrArea + 
    dummy_garage:GarageCars, data = train[-c(524, 633, 1325),])
summary(modelovif2_outliers)
```
...
```{r}
par(mfrow=c(2,2))
plot(modelovif2_outliers)
```
There is still outliers at: 561, 775, 1182. We are going to remove them.
The influencial observations are not out of Cook's distance.

```{r}
modelovif3_outliers<- lm(formula = SalePrice ~ BsmtFinSF1 + TotalBsmtSF + 
    X1stFlrSF + X2ndFlrSF + OpenPorchSF + EnclosedPorch + 
    X3SsnPorch + ScreenPorch + BsmtFullBath + TotRmsAbvGrd + 
    Fireplaces + MSZoning + LotShape + LandContour + LotConfig + 
    Neighborhood + BldgType + OverallQual + OverallCond + RoofStyle + 
    BsmtQual + BsmtExposure + Electrical + KitchenQual + dummy_masvnr:MasVnrArea + 
    dummy_garage:GarageCars, data = train[-c(524,561, 633,775,1182, 1325),])
summary(modelovif3_outliers)
```

As we can not see a signicative improvement in the model, we remain these outliers in the model. We continue with `modelovif2_outliers`
