...
```
```{r}
RMSE(model_interactions$fitted.values, train_validation$SalePrice)
```
...
model_interactionsvif <- lm(formula = SalePrice ~ LotArea + YearRemodAdd + BsmtFinSF1 + 
    GarageArea + WoodDeckSF + BsmtFullBath + FullBath + 
    HalfBath + BedroomAbvGr + Fireplaces + GarageCars + MSSubClass + 
    MSZoning + LotConfig  + Condition1 + HouseStyle + 
    OverallQual + OverallCond + RoofMatl + Exterior1st + Foundation + 
    BsmtCond + Heating + CentralAir + KitchenQual + Functional + 
    SaleCondition + YearRemodAdd:GrLivArea + LotArea:Neighborhood, 
    data = train_train)
vif(model_interactionsvif)
```
Hemos eliminado GrLivArea, Neighborhood por el vif.

## Linearity

```{r, cache=FALSE, message=FALSE,warning=FALSE}
resact <- data.frame(residual = model_interactionsvif$residuals, fitted = model_interactionsvif$fitted.values)
...
```{r}
shapiro.test(model_interactionsvif$residuals)
```
...
```{r}
par(mfrow=c(2,2))
plot(model_interactionsvif)
```
We want to check for the RMSE in validation dataset. We drop Functional and Exterior1st because of poor categories statistical importance.

```{r}
model_new=update(model_interactionsvif, .~.- Functional- Exterior1st)
summary(model_new)

RMSE(model_new$fitted.values, train_train$SalePrice)
RMSE(predict(model_new, newdata=train_validation), train_validation$SalePrice)
```
...
residual_qqnorm <- qqnorm(model_new$residuals)

# Obtener los índices de los valores atípicos
outliers_indices <- which(abs(model_new$residuals) > 3 * sd(model_new$residuals))

# Acceder a los valores atípicos
outliers <- model_new$residuals[outliers_indices]
```

```{r}
model_outliers<- lm(formula = SalePrice ~ LotArea + YearRemodAdd + BsmtFinSF1 + 
    GarageArea + WoodDeckSF + BsmtFullBath + FullBath + HalfBath + 
    BedroomAbvGr + Fireplaces + GarageCars + MSSubClass + MSZoning + 
    LotConfig + Condition1 + HouseStyle + OverallQual + OverallCond + 
    RoofMatl + Foundation + BsmtCond + Heating + CentralAir + 
    KitchenQual + SaleCondition + YearRemodAdd:GrLivArea + LotArea:Neighborhood, 
    data = train_train[-outliers_indices,])
...
```
```{r}
RMSE(model_outliers$fitted.values, train_train[-outliers_indices,]$SalePrice)
