...
With the alias function we delete KitchenAbvGr, HouseStyle, MsSubClass, Electrical, CentralAir, BsmtQual, FireplaceQu, Foundation, Heating, Exterior2, RoofStyle, BsmtExposure and Functional, forming model2.
...
```{r}
shapiro.test(modelovif1$residuals)
qqnorm(residuals(modelovif1))
qqline(residuals(modelovif1))
```
...
```{r}
anova(model2,modelovif1, test="Chisq")
```
The model 1 is better (less variable)


```{r}
confint(modelovif1,level=0.95)
```
...
The outliers are:  1325, 474, 899. We are going to remove them.
The influencial observations are: 524, 629, 1152 (we only delete 524 because is out of Cook's distance)

```{r}
modelovif1_outliers<- lm(formula = SalePrice ~ LotFrontage + LotArea + YearBuilt + dummy_RemodYear:YearRemodAdd + 
    dummy_masvnr:MasVnrArea + dummy_masvnr:MasVnrType + BsmtFinSF1 + 
    BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + X1stFlrSF + X2ndFlrSF + 
    GrLivArea + dummy_garage:GarageCars + dummy_garage:GarageYrBlt + 
    dummy_garage:GarageArea + dummy_garage:GarageType + dummy_garage:GarageFinish + 
    dummy_garage:GarageCond + WoodDeckSF + OpenPorchSF + EnclosedPorch + 
    X3SsnPorch + ScreenPorch + YrSold + BsmtFullBath + BsmtHalfBath + 
    FullBath + HalfBath + BedroomAbvGr + TotRmsAbvGrd + Fireplaces + 
    MoSold + MSZoning + LotShape + LandContour + LotConfig + 
    Neighborhood + BldgType + OverallQual + OverallCond + Exterior1st + 
    ExterQual + ExterCond + BsmtFinType2 + HeatingQC + KitchenQual + 
    BsmtCond + LandSlope + Condition1 + RoofMatl, data = train[-c(474,524,1429,219,899,1325,540,499,561,349,1110,560),])
summary(modelovif1_outliers)
```
...
```{r}
par(mfrow=c(2,2))
plot(modelovif1_outliers)
```
```{r}
shapiro.test(modelovif1_outliers$residuals)
```
...
RMSE(modelovif1_outliers$fitted.values, train$SalePrice)

# RMSE in the test dataset
modelovif1_outliers_pred <- predict(modelovif1_outliers, newdata = test)
RMSE(modelovif1_outliers_pred,modelovif1_outliers$fitted.values)
```
