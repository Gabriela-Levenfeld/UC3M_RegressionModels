...
What should we do with the variables that have more missing values than existing ones? Maintaining them can be a problem as makes no sense to substitute missing values with an statistic of the remaining, such as the mean, as the latter would not be representative. Consequently, we are going to delete the variables, where the percentage of NA is high (>80%). They are: "PoolQC" and the associated numerical one, "PoolArea", "MiscFeature" and "MiscVal" (numerical), "Fence" and "Alley". 
...
To see which are the quantitative features that strongly affect home price, checking the correlation may be an interesting idea.
...
corrplot(correlation_matrix, method = "color", tl.col = "black", title="Correlation Matrix",tl.srt = 0,tl.cex = 0.7, type="upper", diag=FALSE)

```

The response variable, Sale Price, is strongly influenciated by the GrLivArea, GarageArea and GarageCars. Other important variables are FullBath, YearBuilt, YearRemodAdd or TotalBsmtSF.
...
Different transformation have been applied to different variables, they are:
  ...
  - Square root(max(x+1)-x): When there is moderate skew data, this is applied when it is negatively skewed
  
  - Square root: Same  claim as before although in this case is implemented when is positively skewed
  
  - Logarithm: Applied to the variables that have a positive skewness, that are right-tailed. 
  
  - Logarithm (X+1): Same argument as above, but this was done for variables that have zero values, so as to avoid errors. It was applied to 
...
```{r, change of order in category variables, echo=FALSE}
#We change the order of the factors so the baseline factor is "None".
...
Due to the absence of the target variable in the test dataset, we adopted a train-validation split
methodology. This involved dividing our available training data into two subsets: 'train_train'(70%) and
'train_validation'(30%). The 'train_train' subset was utilized for training our predictive model, while the
'train_validation' subset served as a validation set to assess the model's performance.
...
Hemos puesto todas las categorias iguales tanto en traintrain como en train validation y tambien en train y test porque sino sale un error mm, para tener los mismos factores en todas las bases
...
Utilizamos el alias para ver las variables que tienen correlaci�n perfecta entre si y eliminarlas

````{r, echo=FALSE}
alias(model1)
```

THe function *alias()* checks those variables where the correlation between them has either 1 or -1. First we see, correlation for KitchenAbvGr and: MsSubClass and BldgType, so we delete KitchenAbvGr. Computing again the alias we see correlation for MsSubClass and HouseStyle and delete the latter. 
...
Usually, we mark as a significant interaction when p<0.05. However, because we have obtained 30 interactions, we decided to lower such value into 0.001. With this setting (p<0.001) we just find 1 relevant interaction.
...
*Explicar mejor porque nos quedamos con estas dos de las 10 que han salido* 
